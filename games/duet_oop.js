...
class sinRect {
    ...
    move() {
        ...
        if(this.xSinMovement){
            if(this.xPos + sin(this.xSin) > 0 && this.xPos + this.xSize + sin(this.xSin) < width){
...
function setup() {
    ...
    for (let i = 0; i < 100; i++) {
        objects[i] = new sinRect(getRandomInt(0, width - 30), getRandomInt(0, height), 30, 10);
    }
}
